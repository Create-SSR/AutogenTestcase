...
def html_init():
    ...
    # sidebar图标
    st.sidebar.markdown(
        '''<a href="#"><img src='data:image/png;base64,{}' class='img-fluid' width=140 height=40 target='_self'></a>'''.format(
            img_to_bytes("img/logoDZ.png")), unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with expander1:
            st.markdown(
                """
            👉<a href="https://alidocs.dingtalk.com/i/nodes/YMyQA2dXW7r2APq1F6pLAbY78zlwrZgb?utm_scene=person_space" target='blank'>模型ApiKey申请说明</a>👈
            ### **使用步骤**
    ...
    st.sidebar.markdown(
        '''<small style='float: right'>By <a id="reload" href="#Create by WQ" title="Create by WQ">@WQ</a></small>''',
        unsafe_allow_html=True)
    ...
    # main主页面
    source_tab1, source_tab2 = st.tabs(["⚙AI模型设置", "🖥 测试用例生成" ])
    # AI模型设置
    with source_tab1:
        st.subheader("DeepSeek模型配置")
    ...
    with source_tab2:
        ...
        with cols3[0].expander("高级选项"):
            ...
            if show_slider:
                ...
                cases_rate_list = [functional_testing,
                                   ...
                                   perfmon_testing,
                                   regression_testing]
            test_priority = st.selectbox("测试优先级", ["--", "P0", "P1", "P2", "P3"], index=0)
            # 添加测试用例数量控制
        ...
        software_feature = cols3[0].selectbox("软件特性", ["短剧视频播放软件", "小说阅读软件"], index=0)
        # 上传文件
        ...
        system_prompt_message = read_system_message("TESTCASE_PROMPT_SYSTEM_MESSAGE.txt")
        tester_system_message = system_prompt_message.replace("{{functional_testing}}", str(cases_rate_list[0]))\
            ...
            .replace("{{regression_testing}}", str(cases_rate_list[3]))\
            .replace("{{software_feature}}", str(software_feature))
        #
        message_tab1,message_tab2 = cols3[1].tabs(["✍角色","训练"])
        with message_tab1:
            customer_system_message = st.text_area("👉测试工程师角色", height=480, value=tester_system_message)
        with message_tab2:
            pass
        customer_reader_message = system_reader_message
            ...
            if bool(st.session_state.run_cases):
                ...
                if user_input:
                    ...
                    async def m_roles_generate_testcases():
                        ...
                        testcase_writer = get_testcase_writer(model_qwen_client, customer_system_message)
                    ...
                    if eval(conf['qwen']['choice']):
                        if conf['qwen']['api_key'] != "" :
                            try:
                    ...
                    elif eval(conf['qwen']['choice']) and not eval(conf['deepseek']['choice']):
                        if conf['qwen']['api_key'] != "":
                            try:
                                with st.spinner("正在生成测试用例..."):
                            ...
                            st.error("请先配置qwen模型的APIKEY!")
                    else:
                        st.error("请先配置qwen模型并选中保存!")
